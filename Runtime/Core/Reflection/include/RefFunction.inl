#ifndef REF_FUNCTION_INL
#define REF_FUNCTION_INL
#include <type_traits>
...
// 2*2*3*2 = 24
	...
	struct FuncTraits<Func&&> : FuncTraits<Func> {};
	template<typename Func>
	struct FuncTraits<const Func&> : FuncTraits<Func> {};
	template<typename Func>
	struct FuncTraits<const Func&&> : FuncTraits<Func> {};
	...
	private:
		...
		static auto get(Func&& func, std::index_sequence<Ns...>) {
			...
			return[func = std::forward<Func>(func)](Args... args) mutable -> decltype(auto) {
				...
				static_assert(CheckCompatibleArguments<ToArgList, FromArgList>::value,
